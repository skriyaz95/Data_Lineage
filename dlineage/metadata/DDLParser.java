...
public class DDLParser
{
	...
	public DDLParser( Map<TableMetaData, List<ColumnMetaData>> tableColumns,
			...
			EDbVendor vendor, String sqlText, boolean strict, String database )
	{
		this.strict = strict;
		this.vendor = vendor;
	...
	{
		if ( stmt instanceof TCreateTableSqlStatement && ((TCreateTableSqlStatement)stmt).getSubQuery( ) == null)
		{
			TCreateTableSqlStatement createTable = (TCreateTableSqlStatement) stmt;
			parseCreateTable( createTable );
		}
		else if ( stmt instanceof TOracleCommentOnSqlStmt )
		{
		...
		else if ( stmt instanceof TAlterTableStatement )
		{
			...
			{
				tableMetaData.setCatalogName( alterTable.getTableName( )
						.getDatabaseString( ) );
			}
			else
		...
		else if ( stmt instanceof TInsertSqlStatement && ((TInsertSqlStatement)stmt).getSubQuery( ) == null)
		{
	...
	private void parseSelectStmt( TSelectSqlStatement selectStmt )
	{
		...
		impact.setShowUIInfo( true );
		impact.setTraceErrorMessage( false );
		impact.setVirtualTableName( SQLUtil.generateVirtualTableName( selectStmt ) );
		impact.impactSQL( );
		ColumnImpactModel columnImpactModel = impact.generateModel( );
		...
		{
			if ( selectStmt.getTargetTable( ).getTableName( ).getSchemaString( ) != null )
			...
			for ( int i = 0; i < referenceModels.length; i++ )
			{
				...
				if ( referModel.getField( ) != null )
				{
					...
					else if ( resultColumn.getExpr( ) != null )
					{
						...
						if ( removeQuote( resultColumn.getExpr( ).toString( ) ).equalsIgnoreCase( removeQuote( referModel.getField( )
								.getFullName( ) ) ) )
						{
							ColumnMetaData columnMetaData = getColumn( referModel.getColumn( ) );
							if ( !columns.contains( columnMetaData ) )
		...
		for ( int i = 0; i < referenceModels.length; i++ )
		{
			...
			else if ( referModel.getAlias( ) != null )
			{
				if ( resultColumn != null )
				{
					if ( resultColumn.getSchemaString( ) != null )
					{
						if ( removeQuote( resultColumn.toString( ) ).equalsIgnoreCase( removeQuote( referModel.getAlias( )
								.getName( ) ) ) )
						{
							ColumnMetaData columnMetaData = getColumn( referModel.getColumn( ) );
							if ( !columns.contains( columnMetaData ) )
	...
	private void parseCreateIndex( TCreateIndexSqlStatement createIndex )
	{
		...
		if ( createIndex.getIndexName( ) != null )
		{
			...
			{
				for ( int i = 0; i < createIndex.getColumnNameList( ).size( ); i++ )
				{
			...
			TableMetaData tableMetaData )
	{
		...
		{
			for ( int i = 0; i < alterTable.getTableElementList( ).size( ); i++ )
			{
				parseAlterTableElement( alterTable.getTableElementList( )
						.getTableElement( i ), tableMetaData );
			}
		}
	}
		...
		switch ( alterTableOption.getOptionType( ) )
		{
			case AddColumn :
				for ( int i = 0; i < alterTableOption.getColumnDefinitionList( )
						.size( ); i++ )
				{
					parseColumnDefinition( alterTableOption.getColumnDefinitionList( )
							.getColumn( i ),
							tableMetaData );
				}
				break;
			...
			TConstraint constraint )
	{
		...
		{
			TObjectName object = constraint.getColumnList( ).getObjectName( i );
			ColumnMetaData columnMetaData = getColumnMetaData( tableMetaData,
					object );
			columnMetaData.setIndex( true );
			indexColumn indexColumn = new indexColumn( );
			...
			TConstraint constraint )
	{
		...
		if ( constraint.getReferencedObject( ) != null )
			foreignKey.setForeignTable( constraint.getReferencedObject( )
					.toString( ) );
		if ( constraint.getColumnList( ) != null
		...
		if ( columnDef.getColumnName( ) != null )
		{
			...
			if ( columnDef.getDatatype( ) != null )
			{
				...
				if ( type.getScale( ) != null )
				{
					try
					{
						columnMetaData.setScale( Integer.parseInt( type.getScale( )
								.toString( ) ) );
					}
				...
				if ( type.getPrecision( ) != null )
				{
					try
					{
						columnMetaData.setPrecision( Integer.parseInt( type.getPrecision( )
								.toString( ) ) );
					}
				...
				if ( type.getLength( ) != null )
				{
					try
					{
						columnMetaData.setColumnDisplaySize( type.getLength( )
								.toString( ) );
					}
					catch ( NumberFormatException e )
					{
	...
	private void parseCommentOn( TOracleCommentOnSqlStmt commentOn )
	{
		if ( commentOn.getDbObjType( ) == TObjectName.ttobjTable )
		{
			...
			{
				tableMetaData.setCatalogName( commentOn.getObjectName( )
						.getDatabaseString( ) );
			}
			else
				tableMetaData.setCatalogName( database );
		...
		else if ( commentOn.getDbObjType( ) == TObjectName.ttobjColumn )
		{
			...
			{
				tableMetaData.setName( commentOn.getObjectName( )
						.getTableString( ) );
			}
			if ( isNotEmpty( commentOn.getObjectName( ).getSchemaString( ) ) )
			{
				tableMetaData.setSchemaName( commentOn.getObjectName( )
						.getSchemaString( ) );
			}
			if ( isNotEmpty( commentOn.getObjectName( ).getDatabaseString( ) ) )
			{
			...
			if ( index != -1 )
			{
				tableColumns.get( tableMetaData )
						.get( index )
